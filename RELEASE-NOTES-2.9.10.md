...
Existing users do not lose permissions when upgrading. Existing Read grants become Viewer grants, while existing Write grants become Manager grants so that users retain every capability they previously had. The new Editor role can then be used for collaborators who should be able to change content without also controlling access or other administrative settings.
